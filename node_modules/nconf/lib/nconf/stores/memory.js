...
// Replaces root of the storage, useful for working with arrays.
//
Memory.prototype.replace = function (value) {
  if (this.readOnly) {
    return false;
  }

  //
  // Reset the `mtime` (modified time)
  //
  this.mtimes = {};
...
// ### function push (value)
// #### @value {literal|Object} Value to replace with
// Pushes 'value' to the storage, useful for working with arrays.
//
Memory.prototype.push = function (value) {
  if (this.readOnly) {
    return false;
  }
  ...
  // Reset the `mtime` (modified time)
  //
  this.mtimes[this.store.length] = Date.now();;
