...
// {{{ dumps game data file into excel readbale csv
function dump(file)
{
  var output = []
    , data   = new conf.Provider({store: {type: 'file', file: file} });

    // generate header line
    output[0] = [];
    output[0].push('Teams');
    output[0].push('Points');
    // get round numbers
    _.each(data.get('rounds'), function(round, number)
    {
      // we don't need number zero
      if (+number) output[0].push(number);
    });

    // get teams points
    _.each(data.get('teams'), function(team, id)
    {
      var row = [];
      row.push(html_entity_decode(team.short));
      row.push(team.points);

      _.each(data.get('rounds'), function(round, number)
      {
        // we don't need number zero
        if (+number) (round && id in round) ? row.push(round[id]) : row.push(null);
      });
    ...
    fs.open(file.replace(/\.json$/, '.csv'), 'a', function(err, fd)
    {
      if (err) throw err;
      ...
      fs.close(fd, function(err)
      {
        ...
        console.log('Done. ['+file.replace(/\.json$/, '.csv')+']');
      });
    });
...
// {{{ translate html entities into real stuff
// TODO: Make it better
