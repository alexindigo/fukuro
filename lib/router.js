...
var Router =
{
  ...
  'helo': function(data, fn)
  {
    ...
    // hack for audience round,
    // oh Jim you're driving me crazy
      ...
      Object.keys(data).forEach(function(k)
      {
        ...
        if (pl.played)
        {
          Object.keys(pl.played).forEach(function(n)
          {
            ...
            // count points
            players[pl.id].points++;

            // update position
            players[pl.id].pos += round.time;
  ...
  {
    var list = this.game.players() || {};

    if (player)
    {
      // add new one to the list
      list[player.id] = player;
      list[player.id].points = 0;
      list[player.id].pos = 0;
      this.game.players(list);

      this.all.emit('player_new', {total: Object.keys(list).length+1, "new": list[player.id]});
    }
  },
  'player:team': function(data)
  {
    var players = this.game.players();

    // add new one to the list
    if (data && data.id && data.team && players[data.id])
    {
      players[data.id].team = data.team;

      this.all.emit('player_team', {id: data.id, team: data.team});
    }

  },
  'player:answer': function(answer)
  {
    var list = this.game.answers() || {}
      , total = answer.data ? Object.keys(list[answer.data.number] || {}).length : 0
      ;

    // add new one to the list
    if (answer && answer.data)
    {
      if (!list[answer.data.number])
      {
        list[answer.data.number] = {};
      }
      list[answer.data.number][answer.id] = answer;
      this.game.answers(list);

      // add points
      var players = this.game.players();
      if (players[answer.id])
      {
        players[answer.id].points++;
        answer.points = players[answer.id].points;

        players[answer.id].pos += answer.data.time;
        answer.pos = players[answer.id].pos;

        if (players[answer.id].team && !this.game.audienceTeam())
        {
          this.game.audienceTeam({id: answer.id, name: players[answer.id].name, team: players[answer.id].team});
          this.all.emit('player_answer_team', {id: answer.id, name: players[answer.id].name, team: players[answer.id].team});
        }
      }

      this.all.emit('player_answer', {total: total+1, "new": answer});
    }
  },
  //
  // admin commands
  ...
  {
    if (data && (data.item == 'audience_team' || data.item == 'audience_team_team'))
    {
      data.player = this.game.audienceTeam();
    }
  ...
  'admin:sound': function(data, fn)
  {
    var prev;

    if (data.action == 'play')
    {
      if (prev = this.game.sound())
      {
        this.all.emit('sound_stop', prev);
      }
      this.game.sound(data.key);
    }
    ...
    // send sound command to all
    this.all.emit('sound', data);
    if (fn) fn();
  },
  ...
  'admin:scale': function(scale)
  {
    this.game.scale(scale);

    // send sound command to all
    this.all.emit('scale', scale);
  },
  'admin:team': function(data, fn)
  {
    ...
    {
      self.all.emit('team_highlight', data);
    }
    else
  ...
  'admin:team_add_point': function(data, fn)
  {
    var self = this
      , team
      ;
    ...
    // for for admin
    if (!this.socket.id || !this.rooms['/admins'] || !~this.rooms['/admins'].indexOf(this.socket.id))
    {
      // do nothing
      if (fn) fn();
      return;
    ...
    // check for data
    if (!data.id || !(team = this.game.getTeams()) || !(team = team[data.id]))
    {
      // do nothing
      if (fn) fn();
      return;
...
console.log(['team', team]);
    this.game.updatePoints(team.handle, team.points+1, function(team)
    {
      self.all.emit('team', team);
    });
  ...
  'admin:team_remove_point': function(data, fn)
  {
    var self = this
      , team
      ;
  ...
  'sound_stop': function(key)
  {
    this.game.sound(null);
    // send sound command to all
    this.all.emit('sound_stop', key);
  },
  'sound_volume': function(volume)
  {
    // send sound command to all
    this.all.emit('sound_volume', volume);
  }
};
