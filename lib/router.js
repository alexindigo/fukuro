...
var Router =
{
  ...
  'admin:round': function(data)
  {
    // check round and save it
    if (this.game.round() != data.round)
    {
      this.game.round(data.round);
      // notify the gang
      this.socket.broadcast.emit('round', {round: this.game.round()});
    }
  },
  ...
  'off': function(data)
  {
    ...
    if (current = this.game.current())
    {
      ...
      if (current.item != data.item) return;
      if (current.number && (current.number != data.number)) return;

      // clear current
      this.game.current(null);

      // notify others
      this.socket.broadcast.emit('off', data);
    }
  },
