...
var Router =
{
  ...
  'helo': function(data, fn)
  {
    ...
    if (!data.token)
    {
      ...
      this.socket.set('user', token, function(err)
      {
        ...
        fn({teams: this.state.teams, token: token});
      }.bind(this));
    }
    ...
    else
    {
      ...
      {
        // don't really care about errors at this point
        fn({teams: this.state.teams, token: data.token});
      }.bind(this));
    }
  },
  ...
  'answer': function(data, fn)
  {
    ...
    this.socket.get('user', function(err, token)
    {
      ...
      {
        answer = {type: 'audience', player: Players[token].name, answer: Players[token].answer};
        // notify master
