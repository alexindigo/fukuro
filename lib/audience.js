...
var Router =
{
  ...
  'helo': function(data, fn)
  {
    ...
    activeClients++;
console.log(['helo', '# of ' + activeClients]);
    ...
    else
    {
      ...
      this.socket.set('user', data.token, function(err)
      {
        ...
        // don't really care about errors at this point
        fn({status: 'ok', teams: this.state.teams, token: data.token, player: Players[data.token]});
      }.bind(this));
    }
  },
...
console.log(['joined', data.name]);
  ...
  'team': function(data, fn)
  {
    ...
    this.socket.get('user', function(err, token)
    {
      ...
      fn({status: 'ok', token: token});

console.log(['picked team', Players[token].name, data.team]);
  ...
  'answer': function(data, fn)
  {
    ...
    if (!this.state.ready || this.state.timer < 0) return fn({status: 'error'});
    ...
    this.socket.get('user', function(err, token)
    {
      ...
      if (!Players[token].name) Players[token].name = data.name;
      if (!Players[token].team) Players[token].team = data.team;

      // notify client
      fn({status: 'ok'});

      // and start doing our business
      if (Players[token].answer.match(/bell/i) || Players[token].answer.match(/белл/i))
      {
...
console.log(['answered', Players[token].name, Players[token].team, Players[token].answer]);
