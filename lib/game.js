...
var reset = exports.reset = function(callback, doFlush)
{
  // assume we're the only process
  // working with the storage folder
  getStorage($.get('storage'), function(err, storage)
  {
    if (err) throw new exception('Unable to reset storage file. ['+$.get('storage')+']', 500);
    ...
    {
      $D.set('current', $D.get('current') || null);
      $D.set('sound', $D.get('sound') || null);
      $D.set('players', $D.get('players') || {});
      $D.set('answers', $D.get('answers') || {});
      $D.set('scale', $D.get('scale') || 1);
      $D.set('round', $D.get('round') || 0);
      $D.set('rounds', $D.get('rounds') || []);
      $D.set('played', $D.get('played') || []);
      $D.set('audience_team', $D.get('audience_team') || {});
    }

    // set teams list if no set
    if (!$T)
    {
      $T = new store.Provider({store: {type: 'file', file: $.get('teams')} });
    }
    ...
    if (!$C)
    {
      $C = new store.Provider({store: {type: 'file', file: $.get('content')} });
    }
    // or reload from the file
    else
    {
    ...
    // rebuild teams
    if (!$D.get('teams') || doFlush) // real reset
    {
...
// {{{ updates "audience_team"
// audience sponsored team points
exports.audienceTeam = function(player)
{
  var round = $D.get('round')
    , data = $D.get('audience_team');
    ;

  if (typeof player != 'undefined' && !data[round])
  {
    data[round] = player;
    $D.set('audience_team', data);
    // save state (async)
    $D.save();
  }
  return data[round];
...
//    , lastStanding
  ...
  if ((collective = $D.get('flags:collective')) && id != $D.get('flags:guest'))
  {
    ...
    // calculate total
    _.each($D.get('teams'), function(team, key)
    {
      if (team.collective || team.guest) return;

      total += team.points;
      teamsNum++;
    });

    // Changes for Novikov's game
    // collective points = average points of all teams
    collective.points = Math.round(total / teamsNum * 100);

    if (collective.points < 10)
    {
      collective.points = '0.0' + collective.points;
    }
    else if (collective.points < 100)
    {
      collective.points = '0.' + collective.points;
    }
    else
    {
      collective.points = (''+collective.points).replace(/(.{2})$/, '.$1');
    }

    $D.set('rounds:'+round+':'+collective.handle, collective.points);
  ...
  // {{{ groups hack
  // assume there is no mix between collective/guest and groupped teams
  else if (team.group)
  {
    // get actual object
    group = $D.get('teams:'+team.group);

    // calculate total
    _.each($D.get('teams'), function(t, key)
    {
      if (t.group != group.handle) return;
    ...
    // Changes for Novikov's game
    // collective points = average points of all teams
    group.points = Math.round(total / teamsNum * 100);

    if (group.points < 10)
    {
      group.points = '0.0' + group.points;
    }
    else if (group.points < 100)
    {
      group.points = '0.' + group.points;
    }
    else
    {
      group.points = (''+group.points).replace(/(.{2})$/, '.$1');
    }

    $D.set('rounds:'+round+':'+group.handle, group.points);
    $D.set('teams:'+group.handle, group);
    // it's not good pattern, but it's very convenient for now
    // and only call the thing on changes
    callback(group);
  }
...
// used with "audience help" round
exports.updatePoints = function(id, points, callback)
{
  var team = $D.get('teams:'+id)
    , total = 0
    , teamsNum = 0
    ;
